...
#### DELETE ONCE ALL LIBRARIES CONFIRMED
```{r}
...
Then export the CSV
```{r}
write.csv(veglm_sppcor_log, file = "tables/veglm_sppcor_log_PearsonCor.csv")
...
write.csv(veglm_norm_pam_6_part$ctc,file='tables/veglm_norm_pam_6_part$ctc.csv')
...
# Save to a CSV file
write.csv(veglm_norm_pam_6_part$ptc,file='tables/veglm_norm_pam_6_part$ptc.csv')
...
colnames(clus_results_veglm_nc6) <- c("group", "alliance", "ecoregion","ecosite", "plant_community_code", "plant_community_title","total_shrub_prominence", "M1", "M2", "M3", "M4", "M5", "M6", "N", "thresh80_cluster", "maxalpha_cluster", "pam_cluster", "pam_neighbor", "pam_silwidth")

names(clus_results_veglm_nc6)

# save the results to a CSV file
write.csv(clus_results_veglm_nc6,file='tables/clus_results_veglm_nc6.csv',row.names = FALSE)
...
plot(veglm_nc6_part)
write.csv(veglm_nc6_part$ctc,file='tables/veglm_nc6_part$ctc.csv')
...
# Evaluating LM1-APAD with a subset of cluster M1 using sorted tables- Festuca hallii - Hesperostipa_spartea
...
#### Evaluating clusters and communities using shrub prominent
```{r}
...
env_lm <- env_lm %>%
            mutate(alliance_prelim = case_when(
              ...
              plant_community_code == "FFA27" ~ "04_fescam", # reassign FFA27 based on ecoregion and indicator species
...
write.csv(veglm_prelim_alli_part $ctc,file='tables/veglm_prelim_alli_part $ctc.csv')
...
# Save the results to a CSV file for review
write.csv(veg_sppcor_log, file = "tables/veg_sppcor_log_PearsonCor.csv")
...
write.csv(veg_prg_part_ctc,file='tables/veg_prg_part$ctc.csv')
...
write.csv(defuzzify(vegmat_nonref_nz.nc)$cluster,file='tables/vegmat_nonref_nz_nc$cluster.csv')
